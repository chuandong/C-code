	...
	if (pid > 0) {
		while (1) {
			...
			if (-1 == sendto(sockfd, &buff, sizeof(buff), 0,
					(sockaddr_t *)&bcastaddr, sizeof(bcastaddr)))
				error_exit("sendto");
		}
...
int chat_network_init(void)
{
	...
	bcastaddr.sin_port = htons(CHAT_COMM_PORT);
	bcastaddr.sin_addr.s_addr = inet_addr(CHAT_COMM_ADDR);
	if (-1 == bind(sockfd, (sockaddr_t *)&bcastaddr, sizeof(bcastaddr)))
		return -1;
